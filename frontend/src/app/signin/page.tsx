"use client";

import { welcomeImageBlurDataUrl } from "@/data/base64images";
import { signIn } from "next-auth/react";
import Image from "next/image";
import { useState } from "react";
import { Eye, EyeOff, Mail, Lock } from "lucide-react";

export default function Signin() {
  const [formState, setFormState] = useState<"signup" | "signin">("signin");
  const [showPassword, setShowPassword] = useState(false);
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");

  const handleGoogleAuth = async () => {
    await signIn("google", { callbackUrl: "http://localhost:3001/feed" });
  };

  return (
    <div className="min-h-screen relative">
      {/* Background */}
      <div className="absolute inset-0 bg-[#0a0a0a]" />
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[#0d0d0d] via-[#0a0a0a] to-[#080808]" />
      <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: `linear-gradient(rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.03) 1px, transparent 1px)`, backgroundSize: '60px 60px' }} />

      <div className="flex justify-center items-center min-h-screen px-4 py-10 relative z-10">
        <div className="w-full max-w-5xl">
          <div className="bg-[#151515] border border-white/10 rounded-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2">
            
            {/* LEFT FORM SIDE */}
            <div className="p-8 lg:p-12">
              {/* Logo */}
              <div className="flex items-center gap-2 mb-8">
                <Image src="/newlogo.svg" width={40} height={40} alt="logo" />
              </div>

              {/* Heading */}
              <h1 className="text-2xl lg:text-3xl font-bold text-white mb-2">
                {formState === "signup" ? "Create account" : "Welcome back"}
              </h1>
              <p className="text-white/40 text-sm mb-6">
                {formState === "signup" 
                  ? "Join the developer community" 
                  : "Sign in to continue to your account"}
              </p>

              {/* Toggle */}
              <div className="flex items-center gap-2 text-sm mb-8">
                <span className="text-white/40">
                  {formState === "signup" ? "Already have an account?" : "Don't have an account?"}
                </span>
                <button
                  onClick={() => setFormState(formState === "signup" ? "signin" : "signup")}
                  className="text-white hover:text-white/80 font-medium transition-colors"
                >
                  {formState === "signup" ? "Sign in" : "Sign up"}
                </button>
              </div>

              {/* Form */}
              <div className="space-y-4">
                {/* Email Input */}
                <div>
                  <label className="block text-white/60 text-xs font-medium mb-2">Email</label>
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2">
                      <Mail className="w-4 h-4 text-white/30" />
                    </div>
                    <input
                      type="email"
                      className="w-full bg-[#0f0f0f] border border-white/5 rounded-xl pl-10 pr-4 py-3 text-white placeholder-white/20 outline-none focus:border-white/10 transition-colors"
                      placeholder="you@example.com"
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                    />
                  </div>
                </div>

                {/* Password Input */}
                <div>
                  <label className="block text-white/60 text-xs font-medium mb-2">Password</label>
                  <div className="relative">
                    <div className="absolute left-3 top-1/2 -translate-y-1/2">
                      <Lock className="w-4 h-4 text-white/30" />
                    </div>
                    <input
                      type={showPassword ? "text" : "password"}
                      className="w-full bg-[#0f0f0f] border border-white/5 rounded-xl pl-10 pr-12 py-3 text-white placeholder-white/20 outline-none focus:border-white/10 transition-colors"
                      placeholder="••••••••"
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/50 transition-colors"
                    >
                      {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                    </button>
                  </div>
                </div>

                {/* Submit Button */}
                <button className="w-full bg-white text-black font-medium py-3 rounded-xl hover:bg-white/90 transition-colors mt-2">
                  {formState === "signup" ? "Create Account" : "Sign In"}
                </button>

                {/* Divider */}
                <div className="flex items-center gap-4 my-6">
                  <div className="flex-1 h-px bg-white/10" />
                  <span className="text-white/30 text-xs">or</span>
                  <div className="flex-1 h-px bg-white/10" />
                </div>

                {/* Google Button */}
                <button
                  onClick={handleGoogleAuth}
                  className="w-full bg-white/5 border border-white/10 hover:bg-white/10 text-white font-medium py-3 rounded-xl flex items-center justify-center gap-3 transition-colors"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                  </svg>
                  Continue with Google
                </button>
              </div>
            </div>

            {/* RIGHT IMAGE SIDE */}
            <div className="hidden lg:block relative h-full min-h-[500px]">
              <Image
                placeholder="blur"
                blurDataURL={welcomeImageBlurDataUrl}
                src="/welcomeimage2.jpg"
                alt="Welcome"
                fill
                className="object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#151515]/80 via-transparent to-transparent" />
              <div className="absolute top-4 left-4">
                <h2 className="text-black text-2xl font-bold mb-2">Connect. Chat. Build.</h2>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}